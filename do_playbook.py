...
def make_summary_tab(writer, dfs):
    ...
    sum_df = pd.concat([sum_df, 
        df[['local_bucket','local_grad_rate']].groupby(['local_bucket']
            ).median().rename(columns={'local_grad_rate':'median'})],axis=1)
    #75th percentile
    sum_df = pd.concat([sum_df, 
        df[['local_bucket','local_grad_rate']].groupby(['local_bucket']
            ).quantile(.75).rename(columns={'local_grad_rate':'75th'})],
        axis=1)
