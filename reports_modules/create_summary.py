...
def make_summary_tab(
    writer, f_db, dfs, cfg, cfg_sum, campus, debug, summary, sn="Summary"
):
    ...
    for i in range(len(rows)):
        ...
        for c in range(len(master_cols)):
            ...
            new_formula = (
                formula.replace("_r_", sr)
                .replace("_sr_", str(start_row + 1))
                .replace("_er_", str(end_row + 1))
                .replace("_xr_", str(end_row + 2))
            )
    ...
    for i in range(len(master_cols)):
        ...
        if isinstance(width, str):  # an 'h' was appended to the width
            ws.set_column(i, i, float(width[:-1]), f_db["centered"], {"hidden": True})
        else:
