    ...
    for old, new in translation.items():
        if old in string:
            if new.startswith("tbl:"):
                string = string.replace(old, _notnan(data_row[new[4:]], "N/A", "{}"))
            else:
...
def make_pdf_report(fn, dfs, cfg, cfg_ssv, campus, debug, do_solo):
    ...
    for i, stu_data in df.iterrows():
        ...
        # *First row
        name_text = (
            "College application odds report for "
            + stu_data["First"]
            + " "
            + stu_data["Last"]
        )
        ...
        pdf.cell(
            w=w[0], txt="Student's name:", h=h[1], border=1, ln=0, align="L", fill=True
        )
        ...
        txt = igr_label[0] + "GR"
        pdf.cell(w=w[4], txt=txt, h=h[1], border="B", ln=0, align="C", fill=True)

        txt = _notnan(stu_data["local_ideal_gr"], "TBD", "{:2.0%}")
        pdf.cell(w=w[5], txt=txt, h=h[1], border="B", ln=1, align="C", fill=False)
        ...
        _set_color_name(pdf, "light_blue")
        txt = tgr_label[0] + "GR"
        pdf.cell(w=w[4], txt=txt, h=h[2], border="T", ln=0, align="C", fill=True)

        txt = _notnan(stu_data["local_target_gr"], "TBD", "{:2.0%}")
        pdf.cell(w=w[5], txt=txt, h=h[2], border=0, ln=1, align="C", fill=False)
        ...
        pdf.cell(w=w[2], txt=txt, h=h[4], border=0, ln=0, align="C", fill=False)

        txt = "<--Shoot for at least 90% for Money " + tgr_label[0] + "GR"
        pdf.cell(w=sum(w[3:]), txt=txt, h=h[4], border=0, ln=1, align="L", fill=False)
        ...
        pdf.cell(w=w[2], txt=txt, h=h[5], border=0, ln=0, align="C", fill=False)

        txt = "<--Shoot for at least 50% for Money " + igr_label[0] + "GR"
        pdf.cell(w=sum(w[3:]), txt=txt, h=h[5], border=0, ln=1, align="L", fill=False)
        ...
        pdf.cell(
            w=w[0], txt="prospective):", h=h[8], border=0, ln=0, align="L", fill=True
        )
        ...
        if num_apps:
            for j, app_data in stu_apps.iterrows():
                ...
                gr_fill = gr_this < tgr
                pdf.cell(
                    w=w[1], h=h[9], ln=0, txt=gr_text, align="C", fill=gr_fill, border=0
                )
                ...
                for w_, txt_, ln_ in [
                    (
                        w[2],
                        _notnan(app_data["local_odds"] / 100.0, "N/A", "{:2.0%}"),
                        0,
                    ),
                    ...
                    (
                        ...
                        _notnan(
                            app_data[
                                "local_money_code"
                            ],  # (0.8, _notnan(app_data['local_money_code'],
                            "N/A",
                            "{}",
                        ),
                        1,
                    ...
                    pdf.cell(
                        w=w_, h=h[9], txt=txt_, align="C", fill=False, ln=ln_, border=0
                    )
                apps_displayed += 1
        ...
        if school_goals is not None:
            pads, goal_eval = _get_student_goal_performance(
                pdf, w[0], school_goals, goal_descriptions, stu_data, stu_apps, labels
            )
            ...
            pdf.cell(w=w[1], h=h[10], border=0, ln=0, align="C", fill=True, txt="You")
            pdf.cell(w=w[1], h=h[10], border=0, ln=1, align="C", fill=True, txt="Met?")
            ...
            for goal_text, score, judgement in goal_eval:
                ...
                pdf.cell(
                    w=w[1], h=h[10], border=0, ln=0, align="C", fill=False, txt=score
                )
                pdf.cell(
                    w=w[1],
                    h=h[10],
        ...
        if local_cfg["p_footer"]:
            ...
            pdf.cell(
                w=sum(w), h=h[11], border="T", ln=0, align="L", fill=False, txt=p_text
            )

        # Final formatting of lines: bold rects then lines
        pdf.set_line_width(thick_line)
        pdf.rect(left_margin, top_margin + h[0], w[0], sum(h[1:3]))  # around name
        ...
        line_bottom = (
            top_margin
            + sum(h[:9])
            + h[9] * (num_apps if num_apps <= college_max else college_max)
        )
        for x in [1, 2, 4, 5]:
            pdf.line(
                left_margin + sum(w[:x]),
    ...
    if not do_solo:
        ...
        campus_fn = (
            campus + "_single_file_SSVs" + date.today().strftime("_%m_%d_%Y") + ".zip"
        )
...
def _get_student_goal_performance(
    pdf, w, school_goals, goal_descriptions, stu_data, stu_apps, labels
):
    """
...
def _eval_pdf_goal(goal_name, stu_data, stu_apps):
    ...
    if goal_name in ["il_public", "il_public_no_goal"]:
        ...
        return sum((stu_apps.local_odds >= 20) & (stu_apps.local_odds < 50))
    elif goal_name == "match":
        return sum((stu_apps.local_odds >= 50) & (stu_apps.local_odds < 80))
    elif goal_name == "money_reach":
        return sum(
            (stu_apps.local_odds >= 20)
