...
def _predict_preference(grs, odds, default_gr, high_multiplier=2.0):
    ...
    for i in range(len(grs)):
        current_weight = odds.iloc[i] * (
            (grs.iloc[i] - min_grad_rate)
            / (max_grad_rate - min_grad_rate)
            * (high_multiplier - 1.0)
            + 1.0
        )
...
def _calculate_mtgr_migr(tgr, igr, s_app_df):
    ...
    s_app_clean = s_app_df[
        (s_app_df["local_money"] == 1)
...
def create_predictions(roster_df, app_df):
    ...
    for student in roster_df.index:
        ...
        if len(choice_df):  # no need to predict if student has chosen
            ...
            s_app_df["local_odds_adj"] = s_app_df[["local_odds", "local_result"]].apply(
                _adjust_odds, axis=1
            )
            ...
            if len(s_app_clean):  # only calculate if some exist
                ...
                prediction_minus1 = _predict_perfect(grs, odds, default_gr, minus1=True)
                prediction_somepref = _predict_preference(grs, odds, default_gr, 2.0)
                prediction_allequal = _predict_preference(grs, odds, default_gr, 1.0)
    ...
    return pd.DataFrame(return_table[1:], columns=return_table[0]).set_index(
        "StudentID"
    )
...
def add_playbook_calculations(cfg, dfs, debug):
    ...
    df = dfs["roster"].copy()
    df["local_strategy"] = df[["GPA", "local_sat_max"]].apply(
        _get_strategies, axis=1, args=(dfs["Strategies"],)
    )
    df["local_bucket"] = df[["local_strategy", "GPA", "EFC", "Race/ Eth"]].apply(
        _get_bucket, axis=1
    )
    ...
    dfs["roster"] = df.sort_values(
        ["local_bucket", "local_grad_rate"], ascending=[True, False]
    )
...
def add_student_calculations(cfg, dfs, debug):
    ...
    df = dfs["roster"].copy()
    df["subgroup"] = df[["Race/ Eth", "Gender"]].apply(_get_subgroup, axis=1)
    df["local_strategy"] = df[["GPA", "local_sat_max"]].apply(
        _get_strategies, axis=1, args=(dfs["Strategies"],)
    )
    df["local_target_gr"] = df[["local_strategy", "GPA", "EFC", "Race/ Eth"]].apply(
        _get_gr_target, axis=1, args=(dfs["StudentTargets"], "target")
    )
    df["local_ideal_gr"] = df[["local_strategy", "GPA", "EFC", "Race/ Eth"]].apply(
...
def make_students_tab(writer, f_db, dfs, cfg, cfg_stu, campus, debug):
    ...
    for i in range(len(master_cols)):
        ws.conditional_format(
            ...
            {
                "type": "formula",
                "criteria": "=IF(MOD(ROW()-2,4)=0,TRUE,FALSE)",
