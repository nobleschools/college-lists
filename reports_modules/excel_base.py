...
def create_standard_weights_tab(writer, df, format_db):
    ...
    for name, col in names.items():
        wb.define_name(
            sn + name, "=" + sn + "!$" + col + "$2:$" + col + "$" + str(max_row)
        )
    ws.autofilter("A1:E1")
    ws.hide()
...
def create_college_list_lookup_tab(writer, df, format_db):
    ...
    wb.define_name("ExtraCollegeChoice", "=" + sn + "!$A$2:$A$" + str(max_row))
    wb.define_name("ExtraCollegeNCES", "=" + sn + "!$B$2:$B$" + str(max_row))
    wb.define_name("CollegeListLookup", "=" + sn + "!$A$2:$B$" + str(max_row))
    ws.autofilter("A1:C1")
    ws.hide()
...
def create_all_colleges_tab(writer, df, format_db):
    ...
    for name, col in names.items():
        wb.define_name(
            sn[:-1] + name, "=" + sn + "!$" + col + "$2:$" + col + "$" + str(max_row)
        )

    max_col = max(names.values())
    wb.define_name(
        sn[:-1] + "Lookup", "=" + sn + "!$A$2:$" + max_col + "$" + str(len(df) + 1)
    )
...
def create_static_tabs(writer, dfs, formats, cfg, campus, debug, playbook=False):
