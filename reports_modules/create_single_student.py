...
def make_single_tab(writer, f_db, dfs, cfg, cfg_ssv, campus, debug, blank=True):
    ...
    safe_write(ws, 2, 3, "=INDEX(KidIDs,MATCH(E3,LastFirst,0))", f_db["ssv_yellow"])
    ...
    safe_write(
        ...
        5,
        0,
        "=INDEX(ReachStudentTargets,MATCH(D3,KidIDs,0))",
    ...
    safe_write(
        ...
        5,
        3,
        "=INDEX(StudentTargets,MATCH(D3,KidIDs,0))",
        f_db["ssv_yellow_percent"],
    )
    ...
    if not blank:
        if campus in cfg_ssv["print_header"]:  # only do this if spec'ed
            safe_write(ws, 0, 5, cfg_ssv["print_header"][campus], f_db["ssv_date"])
        ws.merge_range(0, 8, 0, 9, top_header, f_db["ssv_counselor_title"])
    safe_write(ws, 1, 4, "Select student's name:", f_db["ssv_student_prompt"])
    safe_write(ws, 2, 4, first_student, f_db["ssv_select_student"])

    ws.data_validation(2, 4, 2, 4, {"validate": "list", "source": "=LastFirst"})

    safe_write(ws, 3, 4, "Odds of 1 or more acceptances to:", f_db["ssv_odds_title"])
    safe_write(
        ws,
        4,
        4,
        '="""Money"" "&TargetGRLabel&" grade rate ("'
    ...
    safe_write(
        ...
        2,
        5,
        '=INDEX(ActualACT,MATCH(D3,KidIDs,0))&"/"&'
        + "INDEX(ActualSAT,MATCH(D3,KidIDs,0))",
        f_db["ssv_act"],
    )
    ...
    safe_write(
        ...
        4,
        6,
        "=1-PRODUCT(AF" + str(d_start + 1) + ":AF" + str(d_end + 1) + ")",
        f_db["ssv_mtgr"],
    )
    safe_write(
        ws,
        5,
        6,
        "=1-PRODUCT(AG" + str(d_start + 1) + ":AG" + str(d_end + 1) + ")",
        f_db["ssv_migr"],
    )

    safe_write(ws, 1, 7, "Race/Eth", f_db["ssv_race_title"])
    safe_write(ws, 2, 7, "=INDEX(KidRace,MATCH(D3,KidIDs,0))", f_db["ssv_race"])
    safe_write(ws, 3, 7, "Goals for #s to the left:", f_db["ssv_odds_title"])
    safe_write(
        ws,
        4,
        7,
        '="<--Shoot for at least 90% for Money "' + "&TGRshortLabel",
        f_db["ssv_mtgr_label"],
    )
    safe_write(
        ws,
        5,
        7,
        '="<--Shoot for at least 50% for Money "' "&IGRshortLabel",
        f_db["ssv_migr_label"],
    )
    ...
    safe_write(
        ws, 1, 9, "=INDEX(ReachStudentTargets,MATCH(D3,KidIDs,0))", f_db["ssv_igr"]
    )
    safe_write(ws, 2, 9, "=INDEX(StudentTargets,MATCH(D3,KidIDs,0))", f_db["ssv_tgr"])
    ...
    safe_write(
        ws, 7, 4, "Schools you might apply to:", f_db["ssv_schools_blank_prompt"]
    )
    safe_write(
        ws,
        7,
        5,
        '=IF(OR(H3="H",H3="B",H3="M",H3="I"),'
        + '"6 yr AA/H Grad Rate","6 yr (all) Grad Rate")',
        f_db["ssv_blank_title"],
    )
    safe_write(ws, 7, 6, "Odds of Admit", f_db["ssv_blank_title"])
    safe_write(ws, 7, 7, "For you, school is a", f_db["ssv_blank_title_small"])
    safe_write(ws, 7, 8, "App Status", f_db["ssv_blank_title"])
    safe_write(ws, 7, 9, "Award code", f_db["ssv_blank_title"])
    ...
    for r in range(b_start, b_end + 1):
        ...
        safe_write(
            ...
            r,
            3,
            "=INDEX(ExtraCollegeNCES,MATCH(E" + r_excel + ",ExtraCollegeChoice,0))",
            f_db["ssv_yellow"],
        )
        safe_write(ws, r, 4, "", f_db["ssv_select_college"])
        ws.data_validation(
            r, 4, r, 4, {"validate": "list", "source": "=ExtraCollegeChoice"}
        )
        safe_write(
            ws,
            r,
        ...
        safe_write(
            ...
            r,
            7,
            "=IF(ISERROR(D" + r_excel + '),"",Z' + r_excel + ")",
            f_db["ssv_gr"],
        )
        safe_write(ws, r, 8, "=IF(ISERROR(D" + r_excel + '),"","N/A")', f_db["ssv_gr"])
        safe_write(
            ws,
            r,
            9,
            "=IF(ISERROR(D"
            + r_excel
            + '),"",INDEX(AllCollegeMoneyCode,MATCH(D'
            + r_excel
            + ",AllCollegeNCES,0)))",
            f_db["ssv_gr"],
        )
        safe_write(
            ...
            r,
            10,
            "=INDEX(AllCollegeBarrons,MATCH(D" + r_excel + ",AllCollegeNCES,0))",
        )
        safe_write(
            ws,
            r,
        ...
        safe_write(
            ...
            r,
            12,
            "=INDEX(AllCollegeSAT25,MATCH(D" + r_excel + ",AllCollegeNCES,0))",
        )
        ...
        safe_write(
            ...
            r,
            15,
            '=INDEX(CustomWeightsGPA,MATCH($H3&":"&$D'
            + r_excel
            + ",CustomWeightsIndex,0))",
        )
        safe_write(
            ws,
            r,
            16,
            '=INDEX(CustomWeightsSAT,MATCH($H3&":"&$D'
            + r_excel
            + ",CustomWeightsIndex,0))",
        )
        safe_write(
            ...
            r,
            17,
            '=INDEX(CustomWeightsIntercept,MATCH($H3&":"&$D'
            + r_excel
            + ",CustomWeightsIndex,0))",
        )
        safe_write(
        ...
        safe_write(
            ...
            r,
            19,
            '=INDEX(CoefficientsGPA,MATCH($H3&":"&$K'
            + r_excel
            + ",CoefficientsIndex,0))",
        )
        safe_write(
            ...
            r,
            20,
            '=INDEX(CoefficientsSAT,MATCH($H3&":"&$K'
            + r_excel
            + ",CoefficientsIndex,0))",
        )
        safe_write(
            ...
            r,
            21,
            '=INDEX(CoefficientsIntercept,MATCH($H3&":"&$K'
            + r_excel
            + ",CoefficientsIndex,0))",
        )
        safe_write(
        ...
        safe_write(
            ...
            r,
            23,
            "=IF(ISNUMBER(S" + r_excel + "),S" + r_excel + ",W" + r_excel + ")",
        )
        safe_write(
            ws,
            r,
        ...
        safe_write(
            ws, r, 28, "=IF(ISNUMBER(Y" + r_excel + "),1-Y" + r_excel + "/100,1)"
        )
    ...
    if not blank:
        ...
        safe_write(
            ...
            r,
            4,
            'Schools currently applying to ("*" indicates ' + "prospective):",
        ...
        safe_write(
            ...
            r,
            5,
            '=IF(OR(H3="H",H3="B",H3="M",H3="I"),"6 yr AA/H Grad Rate",'
            + '"6 yr (all) Grad Rate")',
            f_db["ssv_blank_title_right"],
        )
        safe_write(ws, r, 6, "Odds of Admit", f_db["ssv_blank_title"])
        safe_write(
            ws, r, 7, "For you, school is a", f_db["ssv_blank_title_small_right"]
        )
        ...
        for r in range(s_start, s_end + 1):
            ...
            if r == s_start:  # the first row has a different reference
                safe_write(ws, r, 0, "=A" + str(b_end + 1), f_db["ssv_yellow_percent"])
                safe_write(ws, r, 1, "=B" + str(b_end + 1), f_db["ssv_yellow_percent"])
                ...
                safe_write(
                    ...
                    r,
                    0,
                    "=A" + str(r),  # reference row above
                    f_db["ssv_yellow_percent"],
                )
                safe_write(ws, r, 1, "=B" + str(r), f_db["ssv_yellow_percent"])
                safe_write(ws, r, 2, "=C" + str(r), f_db["ssv_yellow"])
            safe_write(
                ws,
                r,
                3,
                "=IF((ROW()-C" + r_excel + ")<=D4,D5+(ROW()-C" + r_excel + '-1),"")',
                f_db["ssv_yellow"],
            )
            safe_write(
                ws,
            ...
            safe_write(
                ...
                r,
                7,
                "=IF(ISNUMBER(D" + r_excel + "),INDEX(Classes,D" + r_excel + '),"")',
                f_db["ssv_gr"],
            )
            safe_write(
                ws,
                r,
                8,
                "=IF(ISNUMBER(D" + r_excel + "),INDEX(Results,D" + r_excel + '),"")',
                f_db["ssv_gr"],
            )
            safe_write(
                ws,
                r,
                9,
                "=IFERROR(IF(ISNUMBER(D"
                + r_excel
                + "),INDEX(AllCollegeMoneyCode,MATCH(INDEX(NCESids,D"
                + r_excel
                + '),AllCollegeNCES,0)),""),"N/A")',
                f_db["ssv_gr"],
            )
            for c, t in [
                (29, "MGRMult"),  # do the final four columns
                (30, "RGRMult"),
                (31, "MGRMoneyMult"),
                (32, "RGRMoneyMult"),
            ]:
        ...
        if campus in cfg_ssv["school_goals"]:
            safe_write(
                ...
                s_end + 1,
                4,
                "Your list compared to campus goals:",
                f_db["ssv_goals_intro"],
            )
            safe_write(ws, s_end + 2, 4, "Campus Goal", f_db["ssv_goals_t1"])
            safe_write(ws, s_end + 2, 5, "You", f_db["ssv_goals_t2"])
            ws.merge_range(s_end + 2, 6, s_end + 2, 7, "Met?", f_db["ssv_goals_t3"])
            g_start = s_end + 3
    ...
    cond_ranges = [
        ("$F$" + str(b_start + 1) + ":$F$" + str(b_end + 1), str(b_start + 1))
    ]
    if s_start:  # student specific section exists
        cond_ranges.append(
            ("$F$" + str(s_start + 1) + ":$F$" + str(s_end + 1), str(s_start + 1))
        )
        ws.conditional_format(
            "$I$" + str(s_start + 1) + ":$I$" + str(s_end + 1),
            {
                "type": "formula",
